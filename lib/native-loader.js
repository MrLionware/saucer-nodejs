...
function loadNativeBinding() {
    ...
    // Strategy 1: Try prebuilt platform package (npm optionalDependency)
    if (packageName) {
        try {
    ...
    // Strategy 2: Try local build directory
    const localBuildPath = join(__dirname, 'build', 'Release', 'saucer-nodejs.node');
    ...
    // Strategy 3: Try bindings package (development fallback)
    try {
